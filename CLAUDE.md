...
- Add version query strings to bust cache: `styles/style.css?v=20251119204632` — generated by `./bump-version.py`, never edited by hand (see [Development Workflow](#development-workflow))
...
5. **Run `./bump-version.py` before finishing** — see below

### Always bump the version after changing source

**Any change to `src/`, `styles/`, `lib/` or `index.html` must be followed by:**

```bash
./bump-version.py
```

This is not optional and it is not a release step — do it as part of the change, before
reporting the work as done.

**Why.** The app is a PWA with a service worker that caches aggressively, and assets are
requested with `?v=YYYYMMDDHHmmss` query strings. Without a bump, returning users keep
getting the old JavaScript out of the service worker cache: the change looks broken, or
silently does nothing, and it is invisible in a fresh browser profile so it survives
testing. The script rewrites every version query string in `index.html` and increments the
service worker's cache name in `sw.js` (`v25` → `v26`), which is what actually evicts the
old cache.

**Exceptions.** Documentation, tests and the Makefile do not ship to the browser, so a
docs-only or test-only change needs no bump.

Do not hand-edit the version strings; the script keeps `index.html` and `sw.js` in step.
