...
export function runTournamentTests() {
    ...
    runner.test('fromJSON handles missing benchRotation (backward compatibility)', () => {
        const tournament = new Tournament({
            players: ['Alice', 'Bob', 'Charlie', 'Dave'],
            courts: ['Court 1'],
            pointsPerMatch: 16,
        ...
        const json = tournament.toJSON();
        delete json.benchRotation;  // Simulate old data format

        const restored = Tournament.fromJSON(json);

        runner.assertTrue(
            Array.isArray(restored.benchRotation),
            'benchRotation should be initialized for backward compatibility'
        );
    });

    runner.test('fromJSON handles missing benchingMode (backward compatibility)', () => {
        const tournament = new Tournament({
            players: ['Alice', 'Bob', 'Charlie', 'Dave'],
            courts: ['Court 1'],
            pointsPerMatch: 16,
        ...
        const json = tournament.toJSON();
        delete json.benchingMode;  // Simulate old data format

        const restored = Tournament.fromJSON(json);

        runner.assertEqual(
            restored.benchingMode,
            'round-robin',
            'benchingMode should default to round-robin for backward compatibility'
        );
    });
    ...
    runner.test('Configuration accepts valid benchingMode values', () => {
        ...
        runner.assertEqual(tournament1.benchingMode, 'round-robin', 'Should accept round-robin');
        runner.assertEqual(tournament2.benchingMode, 'random', 'Should accept random');
    });

    runner.test('Configuration rejects invalid benchingMode', () => {
        runner.assertThrows(
            () => new Tournament({
                players: ['Alice', 'Bob', 'Charlie', 'Dave'],
                courts: ['Court 1'],
                pointsPerMatch: 16,
                randomize: false,
                benchingMode: 'invalid-mode'
            }),
            'Should throw error for invalid benchingMode'
        );
    });
